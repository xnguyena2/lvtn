...
public class Main {
	...
	static double geDistance(double b1[], double b2[]){
		double maxDistance = 0;
		int length = b1.length;
		for(int i = 0;i < length;i++){
			double currentDistance = Math.abs(b1[i] - b2[i]);
	...
	static void dolechchuan(String parentFolder){
		String[] folderHumanFace = parseDirectory(parentFolder);	
		double[][][] listBio = new double[folderHumanFace.length][][];
		...
		for(int i = 0;i < folderHumanFace.length;i++){
			...
			FaceRecognition face1 = new FaceRecognition();
			listBio[i] = face1.generateBiometric(parentFolder+"\\"+folderHumanFace[i], "jpg");
			//System.out.println("do lech chuan:"+getDoLechChuan(listBio[i]));
		}
		double dlcAvg = 0;
		double[] listDlc = new double[listBio.length];
		for(int i = 0;i<listBio.length;i++){
			double dlc = getDoLechChuan(listBio[i]);
			listDlc[i] = dlc;
			dlcAvg+=dlc;
			System.out.println("same:"+dlc);
		}
		System.out.println("dlctb:"+dlcAvg/listBio.length);
		
		
		dlcAvg = 0;
		double[] lisDlcDif = new double[listBio.length];
		double[][] lisBioAvg = new double[listBio.length][]; 
		for(int i = 0;i<lisBioAvg.length;i++){
			lisBioAvg[i] = getAvgVector(listBio[i]);
		}
		
		for(int i = 0;i<lisBioAvg.length;i++){
			double dlc = getDoLechChuan(lisBioAvg,i);
			lisDlcDif[i] = dlc;
			dlcAvg+=dlc;
			System.out.println("diff:"+dlc);
		}
		System.out.println("dlcdiftb:"+dlcAvg/listBio.length);
		System.out.println("dlcdifffff:"+getDoLechChuan(lisBioAvg));
		
		
		final XYSeries lineDlc = new XYSeries( "DLC Same" );
		final XYSeries lineDlcDif = new XYSeries( "DLC diff" );
		int length = listDlc.length;
		for(int i = 0; i<length;i++){
			lineDlc.add( i , listDlc[i] );
			lineDlcDif.add( i , lisDlcDif[i] );
		}
		final XYSeriesCollection dataset = new XYSeriesCollection( );
		dataset.addSeries( lineDlc );
		dataset.addSeries( lineDlcDif );
		JFreeChart xylineChart = ChartFactory.createXYLineChart(
				"Browser threshold delta!",
				"Category",
				"Score",
		...
		int width = 1024; // Width of the image 
		int height = 480; // Height of the image 
		File XYChart = new File( "DLC.jpeg" );
		try {
			ChartUtilities.saveChartAsJPEG( XYChart, xylineChart, width, height);
		} catch (IOException e) {
	...
	static double[] getAvgVector(double[][] lisBio){
		double[] result = lisBio[0].clone();
		for(int i = 1;i<lisBio.length;i++){
			for(int j = 0;j<result.length;j++){
				result[j]+=lisBio[i][j];
			}
		}
		for(int j = 0;j<result.length;j++){
			result[j]/=lisBio.length;
		}
		return result;
	}
	
	static double getDoLechChuan(double[][] listBio){
		double result = 0;
		double avg = 0;
		double[] avgX = getAvgVector(listBio);
		for(int i = 0;i<listBio.length;i++){
			double dis = geDistance(listBio[i],avgX);
			avg+=dis*dis;
		}
		result = Math.sqrt(avg/listBio.length); 
		return result;
	}
	
	static double getDoLechChuan(double[][] listBio,int index){
		double result = 0;
		double avg = 0;
		for(int i = 0;i<listBio.length;i++){
			if(i!=index){
				double dis = geDistance(listBio[i],listBio[index]);
				avg+=dis*dis;
			}
		}
		result = Math.sqrt(avg/(listBio.length-1));
		return result;
	}
	...
	public static void main(String[] args) {
		...
		dolechchuan("C:\\Users\\Nguyen Phong\\LVTN\\LVTN_SOURCE_BIOMETRIC_AUTHENTICATION\\DSTTClient\\test\\faces94\\faces94\\face");
